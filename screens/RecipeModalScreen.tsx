...
export function RecipeModalScreen({navigation, route}: RootStackScreenProps<"recipeModal">) {
    const [recipeId, setRecipeId] = useState<string>(route.params.id.toString());
    ...
    useEffect(() => {
        (async () => {
            ...
            const {extendedIngredients, title, image, cuisines, dishTypes, instructions} = resp;
            ...
            <View style={styles.recipe}>
                        ...
                        renderItem={({item}) => {
                            return (
                                <View style={{backgroundColor: "#333", margin: 5, padding:10, borderRadius: 10}}>
                                    ...
                                    <Text style={{alignSelf: "center"}}>{item.measures.metric.amount + " " + item.measures.metric.unitShort}</Text>
                                </View>
                            );
                        }}
                        horizontal= {true}
                ...
                <ScrollView style={styles.instructionContainer}>
                    <Text style={styles.instructions}>
                        {recipeInstructions?.replace("<ol>", "").replace("</ol>", "").replace(/<(?:.|\n)*?>/gm, "\n")}
                    </Text>
                </ScrollView>
            </View>
        </View>
